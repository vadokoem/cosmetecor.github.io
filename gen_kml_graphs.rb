...
class Poly_array
  ...
  def generate_kmls
    generate_good_points
    @polys.each do |ko|
      kml = KMLFile.new
      folder = KML::Folder.new(:name => ko.name)
      hg = []
      ko.points.each {|nj| hg << nj.join(',')}
      hg = hg.join(' ')
      plc = KML::Placemark.new(:name => ko.name)
      ...
      folder.features << plc
      kml.objects << folder
      File.open("kml_plates/#{ko.name}.kml", 'w') do |file|
        file.write(kml.render)
      end
    end
  end

  def generate_good_points
    gloom = []
    i = 0
    @polys.each do |ko|
      ko.points.each {|nj| gloom << nj}
      i += 1
    end
    #p gloom[0]
    #p @polys[0].points[0]
    gloom.each do |gl|
      jiji = gl
      gloom.each do |gt|
        if Math::sqrt((jiji[0] - gt[0])**2 + (jiji[1] - gt[1])**2) < 0.1
          x = jiji[0] + gt[0]
          y = jiji[1] + gt[1]
          jiji[0] = gt[0] = x/2
          jiji[1] = gt[1] = y/2
